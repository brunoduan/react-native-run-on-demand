...
public class Arguments {
  ...
  // #ifdef XPENG_BUILD
  /**
   * Converts Facebook's ReadableMap to a Java HashMap<>
   *
   * @param readableMap The Readable Map to parse
   * @return a Java Map<> to be used in memory
   */
  public static HashMap<String, Object> toHashMap(@Nullable ReadableMap readableMap) {
    if (readableMap == null) {
      return null;
    }
  ...
  public static Object toObject(@Nullable ReadableMap readableMap, String key) {
    ...
    switch (readableType) {
      ...
      case String:
        ...
        break;
      case Map:
        result = toHashMap(readableMap.getMap(key));
        break;
      case Array:
        result = toArrayList(readableMap.getArray(key));
        break;
      default:
        throw new IllegalArgumentException("Could not convert object with key: " + key + ".");
    }
  ...
  /**
   * Converts a ReadableArray into a Java ArrayList<>
   *
   * @param readableArray the ReadableArray to parse
   * @return a Java List<> if applicable
   */
  public static ArrayList<Object> toArrayList(@Nullable ReadableArray readableArray) {
    if (readableArray == null) {
      return null;
    }

    ArrayList<Object> result = new ArrayList<>(readableArray.size());
    for (int index = 0; index < readableArray.size(); index++) {
      ...
      switch (readableType) {
        ...
        case String:
          ...
          break;
        case Map:
          result.add(toHashMap(readableArray.getMap(index)));
          break;
        case Array:
          result = toArrayList(readableArray.getArray(index));
          break;
        default:
